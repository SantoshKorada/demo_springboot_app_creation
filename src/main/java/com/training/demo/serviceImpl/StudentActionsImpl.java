...
//		
		StudentResponse studentResponse = StudentResponse.builder().id(rollNo).name(studentRequest.getName())
				.age(studentRequest.getAge()).phoneNumber(studentRequest.getPhoneNumber()).build();
	...
	public StudentResponse updateStudent(StudentRequest studentRequest, Integer id) {
		...
		for (StudentResponse studentResponse : studentsResponseList) {

			if (studentResponse.getId().equals(id)) {
	...
	public void deleteStudent(Integer id) {
		...
		for (StudentResponse studentResponse : studentsResponseList) {
			if (studentResponse.getId().equals(id)) {
				studentsResponseList.remove(studentResponse);
			}
	...
	public static void main(String args[]) {
		...
		StudentRequest SantoshRequest = StudentRequest.builder().name("Santosh").age(21).phoneNumber(90300303L).build();

		studentActions.addStudent(SantoshRequest);
		studentActions.getAllStudents();

		StudentRequest nikithaRequest = StudentRequest.builder().name("Nikitha").age(22).phoneNumber(567890L).build();
